...
# CHANGE: import fixed project paths from config instead of asking the user
from src.config import RAW_DATA_DIR, PROCESSED_DATA_DIR


def main():
    """Runs data processing scripts to turn raw data into cleaned train/test datasets."""
    logger = logging.getLogger(__name__)
    logger.info("making final data set from raw data")

    # CHANGE: use fixed input paths from config instead of click.prompt(...)
    input_filepath_users = RAW_DATA_DIR / "usagers-2021.csv"
    input_filepath_caract = RAW_DATA_DIR / "caracteristiques-2021.csv"
    input_filepath_places = RAW_DATA_DIR / "lieux-2021.csv"
    input_filepath_veh = RAW_DATA_DIR / "vehicules-2021.csv"

    # CHANGE: automatically create/use the output folder from config
    PROCESSED_DATA_DIR.mkdir(parents=True, exist_ok=True)

    # CHANGE: pass the configured output folder directly
    process_data(
...
def process_data(input_filepath_users, input_filepath_caract, input_filepath_places, input_filepath_veh, output_folderpath):
    ...
    # CHANGE: ensure the output folder exists using Path.mkdir()
    output_folderpath.mkdir(parents=True, exist_ok=True)

    # CHANGE: save files using Path objects instead of os.path.join()
...
if __name__ == "__main__":
    ...
    # CHANGE: removed unused project_dir and click-based CLI entry
    main()